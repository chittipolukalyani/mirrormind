...
export default function Session() {
  ...
  const startSession = async () => { setStarted(true); await loadQuestion(0); };
  ...
  const loadQuestion = async (num: number) => {
    ...
    } catch { setError("Could not connect to backend. Make sure it's running."); }
  };
  ...
  const getAnalysis = async (msgs: Message[]) => {
    ...
    } catch { setError("Analysis failed. Please try again."); setPhase("session"); }
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#080810", color: "#f0eee8", fontFamily: "'Inter', sans-serif", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "24px" }}>
      ...
      <button onClick={() => router.push("/")} style={{ position: "fixed", top: "24px", left: "24px", background: "transparent", border: "1px solid rgba(255,255,255,0.07)", color: "#7a7a92", padding: "8px 16px", borderRadius: "100px", fontSize: "0.8rem", cursor: "pointer", fontFamily: "'DM Mono', monospace" }}>← Back</button>

      <AnimatePresence mode="wait">

        {!started && (
          <motion.div key="start" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} style={{ textAlign: "center", maxWidth: "500px" }}>
            ...
            <p style={{ color: "#7a7a92", fontWeight: 300, lineHeight: 1.7, marginBottom: "48px" }}>5 questions. 2 minutes. Complete honesty.<br />You can answer by voice or by typing.</p>
            <button onClick={startSession} style={{ background: "linear-gradient(135deg, #9b7fe8, #c084fc)", color: "#fff", border: "none", padding: "16px 40px", borderRadius: "100px", fontSize: "1rem", fontWeight: 600, cursor: "pointer", boxShadow: "0 0 40px rgba(155,127,232,0.4)" }}>Begin my session →</button>
          </motion.div>
        )}

        {started && phase === "session" && (
          <motion.div key="session" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} style={{ width: "100%", maxWidth: "640px" }}>
            <div style={{ marginBottom: "40px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px" }}>
                <span style={{ fontFamily: "'DM Mono', monospace", fontSize: "0.65rem", color: "#9b7fe8", letterSpacing: "0.1em", textTransform: "uppercase" }}>Question {questionNum + 1} of {totalQuestions}</span>
                <span style={{ fontFamily: "'DM Mono', monospace", fontSize: "0.65rem", color: "#3a3a52" }}>{Math.round(((questionNum + 1) / totalQuestions) * 100)}%</span>
              </div>
              <div style={{ height: "2px", background: "#13131f", borderRadius: "1px", overflow: "hidden" }}>
                <motion.div style={{ height: "100%", background: "linear-gradient(90deg, #9b7fe8, #c084fc)", borderRadius: "1px" }} animate={{ width: `${((questionNum + 1) / totalQuestions) * 100}%` }} transition={{ duration: 0.5 }} />
              </div>
            </div>

            <motion.div key={questionNum} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} style={{ background: "#13131f", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "20px", padding: "36px", marginBottom: "24px" }}>
            ...
            <div style={{ display: "flex", gap: "8px", marginBottom: "16px" }}>
              {(["voice", "text"] as const).map((mode) => (
                <button key={mode} onClick={() => { setInputMode(mode); setTranscript(""); setTypedAnswer(""); if (isListening) stopListening(); }}
                  style={{ padding: "8px 20px", borderRadius: "100px", border: "1px solid", borderColor: inputMode === mode ? "#9b7fe8" : "rgba(255,255,255,0.07)", background: inputMode === mode ? "rgba(155,127,232,0.15)" : "transparent", color: inputMode === mode ? "#c084fc" : "#7a7a92", fontSize: "0.8rem", fontWeight: 500, cursor: "pointer", fontFamily: "'DM Mono', monospace", letterSpacing: "0.05em", textTransform: "uppercase" }}>
            ...
            {inputMode === "voice" && (
              <div style={{ background: "#13131f", border: `1px solid ${isListening ? "rgba(155,127,232,0.5)" : "rgba(255,255,255,0.07)"}`, borderRadius: "20px", padding: "24px", marginBottom: "16px", minHeight: "100px" }}>
                {transcript ? <p style={{ color: "#f0eee8", fontSize: "0.95rem", lineHeight: 1.6, fontWeight: 300 }}>{transcript}</p>
            ...
            {inputMode === "text" && (
              <textarea value={typedAnswer} onChange={(e) => setTypedAnswer(e.target.value)} placeholder="Type your answer here..." rows={4}
                style={{ width: "100%", background: "#13131f", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "20px", padding: "24px", color: "#f0eee8", fontSize: "0.95rem", lineHeight: 1.6, fontFamily: "'Inter', sans-serif", fontWeight: 300, marginBottom: "16px", display: "block" }} />
            )}
            ...
            <div style={{ display: "flex", gap: "12px", alignItems: "center", justifyContent: "center" }}>
              {inputMode === "voice" && (
                <button onClick={isListening ? stopListening : startListening} style={{ width: "64px", height: "64px", borderRadius: "50%", background: isListening ? "linear-gradient(135deg, #ef4444, #dc2626)" : "linear-gradient(135deg, #9b7fe8, #c084fc)", border: "none", cursor: "pointer", fontSize: "1.5rem", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: isListening ? "0 0 30px rgba(239,68,68,0.4)" : "0 0 30px rgba(155,127,232,0.4)" }}>
                  {isListening ? "⏹" : "🎙️"}
                </button>
              )}
              {hasAnswer && !isListening && (
                <motion.button initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} onClick={submitAnswer}
                  style={{ background: "linear-gradient(135deg, #9b7fe8, #c084fc)", color: "#fff", border: "none", padding: "16px 32px", borderRadius: "100px", fontSize: "0.95rem", fontWeight: 600, cursor: "pointer" }}>
        ...
        {phase === "results" && analysis && (
          <motion.div key="results" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} style={{ width: "100%", maxWidth: "720px" }}>
            ...
            <div ref={shareCardRef} style={{ background: "#080810", padding: "48px", borderRadius: "24px", border: "1px solid rgba(155,127,232,0.2)", marginBottom: "32px" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "40px" }}>
                ...
                <div style={{ fontFamily: "'DM Mono', monospace", fontSize: "0.65rem", color: "#3a3a52", letterSpacing: "0.1em", textTransform: "uppercase" }}>Perception Profile</div>
              </div>
              ...
              <div style={{ textAlign: "center", marginBottom: "40px" }}>
                <div style={{ display: "inline-flex", flexDirection: "column", alignItems: "center", background: "#13131f", border: "1px solid rgba(155,127,232,0.2)", borderRadius: "20px", padding: "20px 36px", marginBottom: "24px" }}>
                  <span style={{ fontFamily: "'Playfair Display', serif", fontSize: "3rem", fontWeight: 700, color: "#9b7fe8" }}>{analysis.score}</span>
                ...
                <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: "clamp(1.2rem, 3vw, 1.8rem)", fontWeight: 700, lineHeight: 1.3, maxWidth: "560px", margin: "0 auto" }}>{analysis.headline}</h2>
              </div>
            ...
            <div style={{ display: "flex", gap: "16px", justifyContent: "center", flexWrap: "wrap" }}>
              <button onClick={handleShare} disabled={sharing} style={{ background: "linear-gradient(135deg, #9b7fe8, #c084fc)", color: "#fff", border: "none", padding: "14px 32px", borderRadius: "100px", fontSize: "0.9rem", fontWeight: 600, cursor: "pointer", boxShadow: "0 0 20px rgba(155,127,232,0.3)", opacity: sharing ? 0.7 : 1 }}>
